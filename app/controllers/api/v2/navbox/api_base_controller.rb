class Api::V2::Navbox::ApiBaseController < ApplicationController
  ...
  def authenticate_app_token!
    return if AppToken.where(active: true, platform: 'navbox', token: request.headers['X-APP-TOKEN']).exists?
